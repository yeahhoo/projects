...
public class DataSourceConfig {
    ...
    public Server h2Server() throws SQLException {
        return Server.createTcpServer("-tcp", "-tcpAllowOthers", "-tcpPort", inMemoryDatabaseTcpPort);
    }

    @Bean(initMethod="start", destroyMethod="stop")
    @DependsOn(value = "h2Server")
    public Server h2WebServer() throws SQLException {
        Server server = Server.createWebServer("-web", "-webAllowOthers", "-webPort", inMemoryDatabaseWebServerPort);
        return server;
    }
